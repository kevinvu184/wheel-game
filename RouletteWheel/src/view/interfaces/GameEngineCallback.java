 ...
 * events<br>
 * i.e. the slot that the ball has entered as the wheel spins and the final
 * result when it stops
 * 
 * @author Caspar Ryan
 * 
 */
public interface GameEngineCallback {
	 ...
	 * @param engine - a convenience reference to the engine so the receiver can
	 ...
	 * this is a convenient place to call
	 * {@link GameEngine#calculateResult(Slot winningSlot)}<br>
	 * and {@link Player#resetBet()}
	 * 
	 * @param winningSlot - the slot that the ball landed in
	 * @param engine      - a convenience reference to the engine so the receiver
	 *                    can call methods if necessary
	 * @see model.interfaces.GameEngine
	 */
